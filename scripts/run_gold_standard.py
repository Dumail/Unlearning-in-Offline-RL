"""Gold Standard: retrain DT from scratch on retain set, evaluate TMI.

Usage:
    uv run python scripts/run_gold_standard.py                    # halfcheetah, seed=42
    uv run python scripts/run_gold_standard.py env=hopper_mr      # hopper
    uv run python scripts/run_gold_standard.py seed=0             # specific seed
"""
...
def create_model(cfg, device):
    ...
    if not hasattr(model, "to"):
        ...
        raise TypeError(
            f"Model '{model_name}' is not yet supported by run_gold_standard.py. "
...
def main(cfg: DictConfig) -> None:
    ...
    print(f"=== Gold Standard: {env_name}, seed={seed} ===")
    ...
    # Train from scratch on D_r
    model = create_model(cfg, device)
    ckpt_dir = Path(cfg.checkpoint_dir) / "gold_standard" / env_name / f"seed_{seed}"
    ...
    results_path = results_dir / f"gold_standard_seed{seed}.json"
    with open(results_path, "w") as f:
        json.dump(results, f, indent=2)
    print(f"\nResults saved to {results_path}")

    # Gate check
    if tmi_results["gold_standard_valid"]:
        print("\n*** GATE PASSED: Gold Standard is valid ***")
        print(f"  AUC: {tmi_results['forget_auc']:.4f}")
        print(f"  Permutation p-value: {tmi_results['forget_auc_pvalue']:.4f}")
    else:
        print("\n*** GATE FAILED: Gold Standard validity check failed ***")
        print(f"  AUC: {tmi_results['forget_auc']:.4f}")
        print(
            f"  Permutation p-value: {tmi_results['forget_auc_pvalue']:.4f} (need > 0.05)"
        )
        print(f"  CI width: {tmi_results['forget_auc_ci_width']:.4f}")
